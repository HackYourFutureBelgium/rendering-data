{ // this file is block-scoped for a cleaner view in the debugger
  ...
  */
  const runTestCase = (solution, reference, args, name = '') => {
    ...
    const beginPadding = Array.from(begin).map(() => '-').join('');
    console.log(`%c${beginPadding}\n${begin}\n${beginPadding}`, 'font-weight:bold;');
    ...
    console.log('%c\n--- rendering your solution (actual):', 'font-weight:bold;');
    let actual = null;
    try {
    ...
    console.log('%c\n--- rendering the reference (expected):', 'font-weight:bold;');
    let expected = null;
    try {
    ...
    if (actual instanceof Element && expected instanceof Element) {
      console.log('%c\n--- comparing actual and expected:', 'font-weight:bold;');
      compareElements(actual, expected);
    }
    else {
      console.log('%c\n--- cannot compare actual and expected:', 'font-weight:bold;');
    ...
    console.log('%c\n--- appending container to the DOM ...', 'font-weight:bold;');
    document.getElementById('test-cases').appendChild(testCaseContainer);

    const end = `=== === === END: test ${name} === === ===`;
    const endPadding = Array.from(end).map(() => '-').join('');
    console.log(`%c\n${endPadding}\n${end}\n${endPadding}\n\n\n\n`, 'font-weight:bold;');
  ...
  const renderBoth = (actual, expected, name, args) => {
...

    {
      ...
      inDebugger.innerHTML = name + ': inspect properties';
      inspectContainer.appendChild(inDebugger);
    }

    // superfluous block scoping is for a cleaner view in the debugger
    // when this onclick is triggered
    testContainer.firstChild.firstChild.onclick = () => eval(`debugger;`);
   ...
   * @returns {undefined}
   */
  const compareElements = (actual, expected, preface, nested) => {

    const expect = chai.expect;

    const name = typeof preface !== 'string'
      ? `actual:   ${elToString(actual)} \nexpected: ${elToString(expected)}`
      : `${preface}  actual:   ${elToString(actual)}\n  expected: ${elToString(expected)}`;
    ...
    describe(name, () => {
      ...
      if (expected.tagName === 'CANVAS') {
        ...
        const expectedImageData = expected.getContext('2d').getImageData(0, 0, expected.width, expected.height);

        const sameDimensions = actualImageData.length === expectedImageData.length
      ...
      const actualDirectTextContent = Array.from(actual.childNodes)
        .reduce(computeDirecTextContent, '');
      const expectedDirectTextContent = Array.from(expected.childNodes)
        .reduce(computeDirecTextContent, '');
      if (actualDirectTextContent !== expectedDirectTextContent
      ...
      if (actualClassList.length !== 0 || expectedClassList.length !== 0) {
        const renderedClassListMembers = expectedClassList
          .reduce((msg, next, index) => {
      ...
      // test attributes
      const actualAttributes = Array.from(actual.attributes).map(item => item.name);
      const expectedAttributes = Array.from(expected.attributes).map(item => item.name);
      const attributesToTest = Array.from(new Set([...actualAttributes, ...expectedAttributes]));

      for (const attr of attributesToTest) {

        const baseLineValue = baseLineEl.getAttribute(attr);
        const isBaselineValue = actual.getAttribute(attr) === baseLineValue
        ...
        if (expected.hasAttribute(attr)) {
          ...
          it(`should have attribute - ${renderedAttribute}`, () => {
            expect(actual).to.have.attribute(attr, expected.getAttribute(attr));
          });
      ...
      const propertiesToTest = Array.from(new Set([...actualProperties, ...expectedProperties]));
      ...
      for (const prop of propertiesToTest) {
        ...
        if (Object(baseLineValue) === baseLineValue) {
          ...
          isBaselineValue = Number.isNaN(actual[prop])
            && Number.isNaN(expected[prop]);
        }
        else {
          isBaselineValue = actual[prop] === baseLineValue
        ...
        if (prop in expected) {
          const renderedProperty = typeof expected[prop] === 'string'
            ? '"' + expected[prop] + '"'
            : expected[prop];
      ...
      if (actual.childElementCount !== 0 || expected.childElementCount !== 0) {
        // assert how many there should be
        it(`should have property - childElementCount: ${expected.childElementCount}`, () => {
          expect(actual.childElementCount).to.equal(expected.childElementCount);
        });
        ...
        if (actual.childElementCount === expected.childElementCount
          ...
          for (let i = 0; i < expected.childElementCount; i++) {
            ...
            compareElements(nextActual, nextExpected, `.children[${i}]\n`, true);
          };
        }
      }
    });
  ...
  for (const testCase of testCases) {
    ...
    runTestCase(solution, reference, testCase, index);
  }
};
